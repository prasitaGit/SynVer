...
Ltac progltac g' fname freefuncName count :=
  ...
  | Forall ?f' (sortarr ?l') => match goal with 
                                | [H: Forall f' l' |- _] => apply relationPreservedSorted in H; try assumption
                                | [H: _ |- _] => idtac
                                end
  | Forall ?f' ?l => simplifyLists; tryif solve[list_solve | sortedSolve f' l] then idtac else first[progress (fireHypothesisOnSublist fname) | fireAppropriateHypothesis fname; fireHypothesisOnSublist fname]; progltac g fname freefuncName (count + 1)
  ...
  | NoDup (sortarr ?l') => match goal with 
                           | [H: NoDup l' |- _] => apply noDupSorted in H; try assumption
                           | [H: _ |- _] => idtac
                           end
  | NoDup ?l' => match goal with (*reverse*)
                 | [H: NoDup (sortarr l') |- _] => apply noDupSorted; try assumption
                 | [H: _ |- _] => idtac
                 end
  | repable_signed ?a => unfold repable_signed; progltac g fname freefuncName (count + 1)
  | _ <= ?a < _ => tryif solve [rep_lia | list_solve] then idtac else (tryif progress (fireAppropriateConclusion) then progltac g fname freefuncName (count + 1) else idtac) 
  | _ < ?a <= _ => tryif solve [rep_lia | list_solve] then idtac else (tryif progress (fireAppropriateConclusion) then progltac g fname freefuncName (count + 1) else idtac)  
  ...
  | _ <= ?a <= _ =>  tryif solve [rep_lia | list_solve | apply byteZRelation; first[rep_lia | list_solve]] then idtac else (tryif progress (fireAppropriateConclusion) then progltac g fname freefuncName (count + 1) else idtac) 
  ...
  | context [Zlength (sortarr _)] => rewrite !lengthPreservedSorted; progltac g fname freefuncName (count + 1)
  ...
  | isSkewed (insListSpec _ E) => apply treeSkewed; progltac g fname freefuncName (count + 1)
  | sorted (sortarr ?l) => apply sort_sorted
