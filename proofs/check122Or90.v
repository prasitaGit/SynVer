...
Require Import GenProof.check122Or90C.
...
Definition check122Or90_spec : ident * funspec :=
  DECLARE _check122Or90
   ...
   POST [ tbool ]
    EX bv : bool, 
    PROP (((In 122 (sublist ind length contents)) \/ (In 90 (sublist ind length contents))) <-> (bv = true)) 
    RETURN (bool2val bv)
    SEP(data_at sh (tarray tint length) (map Vint (map Int.repr contents)) a).

Definition Gprog := [malloc_spec; free_spec; check122Or90_spec].

Lemma check122Or90Synth: semax_body Vprog Gprog f_check122Or90 check122Or90_spec.
Proof.
Admitted.