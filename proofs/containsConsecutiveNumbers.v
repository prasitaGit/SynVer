...
Require Import GenProof.containsConsecutiveNumbersC.
...
Definition containsConsecutiveNumbers_spec : ident * funspec :=
  DECLARE _containsConsecutiveNumbers
    WITH a: val, sh : share, contents : list Z, ind : Z, length : Z
     ...
     POST [ tbool ]
      EX b : bool,
     PROP ((Forall2 (fun x y => Z.add x 1 <> y) (sublist ind (length - 1) contents) (sublist (ind + 1) length contents)) <-> (b = false)) 
     RETURN (bool2val b) 
     SEP (data_at sh (tarray tint length) (map Vint (map Int.repr contents)) a).

Definition Gprog := [malloc_spec; free_spec; containsConsecutiveNumbers_spec].

Lemma containsConsecutiveNumbersSynth: semax_body Vprog Gprog f_containsConsecutiveNumbers containsConsecutiveNumbers_spec.
Proof.
Admitted.